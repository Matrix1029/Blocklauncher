...
ModPE.langEdit("menu.copyright", "© Mojang AB | §fH§falcyon");
...
var errorMessage = (error) => {
    if (Halcyon.isDev) clientMessage("§f[§fH§f] §e" + error.toString() + " §f(§l§o§7" + error.lineNumber + "§r§f)");
...
associateModule("KillAura,K", [!1, !1], "Combat", [{
    ...
    name: "Mode",
    type: "string",
    value: "Multi,Single,Switch,Multi"
}], {
    toggled: false,
    last: 0,
    target: -1,
    ...
    attack(a, v) {
        ...
        if (getState(mod) && a == getPlayerEnt()) {
            if (getCode(mod).getDistance(v) <= range) {
                target = v
            }
        }
    },
...
associateModule("HitAim,HA", [!1, !1], "Combat", 0, {
    attack(a, v) {
...
associateModule("Follow,F", [!1, !1], "Combat", [{
    name: "From EntityList",
    type: "bool",
    value: true
}], {
    ...
    isInWater() {
        var mod = getModule("Follow");
        if (getCode(mod).isLiquid(getTile(getPlayerX(), getPlayerY() - 1.81, getPlayerZ()))) return true;
        return false;
    },
    onground: function () {
        var mod = getModule("Follow");
    ...
    tick(a, v) {
        ...
        if (getState(mod)) {
            ...
            if (target != -1 && target != null) {
                ...
                if (getCode(mod).onground() && getPlayerY() - 1.62 - (Entity.getEntityTypeId(target) == 63 ? Entity.getY(target) - 1.62 : Entity.getY(target)) < -0.8)
                    setVelY(getPlayerEnt(), 0.42);
                if (getCode(mod).isInWater() && getPlayerY() - 1.62 - (Entity.getEntityTypeId(target) == 63 ? Entity.getY(target) - 1.62 : Entity.getY(target)) < -0.8)
    ...
    tick() {
        ...
        if (getState(mod) && getCode(mod).target != -1) {
            ...
            if (Entity.getX(target) != 0 && Entity.getZ(target) != 0) {
                if (getPlayerX() + (dirV.x * 3) - Entity.getX(getPlayerEnt()) > 0.5 || getPlayerX() + (dirV.x * range) - Entity.getX(getPlayerEnt()) < -0.5 || getPlayerY() + (dirV.y * range) - Entity.getY(getPlayerEnt()) > 0.5 || getPlayerY() + (dirV.y * range) - Entity.getY(getPlayerEnt()) < -0.5 || getPlayerZ() + (dirV.z * range) - Entity.getZ(getPlayerEnt()) > 0.5 || getPlayerZ() + (dirV.z * range) - Entity.getZ(getPlayerEnt()) < -0.5) {
                    Entity.setVelX(getPlayerEnt(), (Entity.getX(target) + (dirV.x * range) - Entity.getX(getPlayerEnt())) / 5 * vel);
                    //   Entity.setVelY(getPlayerEnt(), (Entity.getY(target) + (dirV.y * range) - Entity.getY(getPlayerEnt())) / 5);
                    Entity.setVelZ(getPlayerEnt(), (Entity.getZ(target) + (dirV.z * range) - Entity.getZ(getPlayerEnt())) / 5 * vel);
                } else {
        ...
        if (getState(mod)) {
            if (lock) {
                ...
                if (getCode(mod).target != null && getCode(mod).target != -1 && getCode(getModule("EntityList")).isValid(getCode(mod).target)) {
                    ...
                    if (x != 0 && y != 0 && z != 0) {
                        Entity.setPosition(getPlayerEnt(), (x + Math.sin(yaw) * gap), y + Math.sin(pitch) * gap, (z - Math.cos(yaw) * gap));
...
associateModule("BunnyHop,BH", [!1, !1], "Movement", [{
    name: "Auto Move",
    type: "bool",
    value: true
},{
    ...
    tick() {
        var mod = getModule("BunnyHop");
        var yvel = getSettings(mod, "Y-Velocity", 0);
        var xvel = getSettings(mod, "X-Velocity", 0);
        var sett = getSettings(mod, "Auto Move", 0);
        if (getState(mod) && getCode(mod).onground()) {
            setVelY(getPlayerEnt(), +yvel);
            if(sett){
			...
			var zz = Math.cos(p);
			setVelZ(getPlayerEnt(), xvel * yy);
            setVelX(getPlayerEnt(), xvel * xx);
        }
        }
    }
});
    ...
    button() {
        ...
        uithread(() => {
            try {
                ...
                getCode(mod).gui.setAnimationStyle(android.R.style.Animation_Toast);
...
associateModule("Move,Mv", [!1, !1], "Movement", [{
    ...
    jump() {
        ...
        /*var speed = parseFloat(sett);
            ...
            toDirectionalVector(getCode(mod).playerDir, (getYaw() + 90) * (Math.PI / 180), getPitch() * (Math.PI / 180) * -1);
            var player = getPlayerEnt();
            setPosition(getPlayerEnt(), Player.getX() + (speed * getCode(mod).playerDir[0]), Player.getY() + (speed * getCode(mod).playerDir[1]), Player.getZ() + (speed * getCode(mod).playerDir[2]))
        ...
        setVelY(Player.getEntity(), 0);
        setVelZ(Player.getEntity(), 0);*/
        Entity.setPositionRelative(getPlayerEnt(), x * sett, y * sett, z * sett)
        /*var mod = getModule("Move");
        var sett = getSettings(mod, "Distance", 0);
            ...
            var player = getPlayerEnt();
            setPosition(getPlayerEnt(), Player.getX() + (speed * playerDir[0]), Player.getY() + (speed * playerDir[1]), Player.getZ() + (speed * playerDir[2]))

            Entity.setCollisionSize(getPlayerEnt(), 0.6, 1.8);
        //setVelY(getPlayerEnt(), +sett);*/
        /*toDirectionalVector(playerDir, (getYaw() + 90) * Math['PI'] / 180, getPitch() * Math['PI'] / 180 * -1);
        setVelX(getPlayerEnt(), movementTab[11].setting.split(';')[0].split(':')[1] * playerDir[0]);
        setVelZ(getPlayerEnt(), movementTab[11].setting.split(';')[0].split(':')[1] * playerDir[2]);
...
associateModule("Glide,G", [!1, !1], "Movement", [{
    ...
    tick() {
        ...
        var sett = getSettings(mod, "Velocity", 0);
        if (getState(mod)) {
            if (type == "Down") setVelY(getPlayerEnt(), -sett);
            if (type == "Up") setVelY(getPlayerEnt(), sett);
            if (type == "No Y-Motion") setVelY(getPlayerEnt(), 0);
        }
    }
});
...
associateModule("Jesus,Js", [!1, !1], "Movement", 0, {
    ...
    tick() {
        ...
        if (getState(mod)) {
            ...
            } else if ((getTile(getPlayerX(), getPlayerY() - 1.68, getPlayerZ()) >= 8 && getTile(getPlayerX(), getPlayerY() - 1.68, getPlayerZ()) <= 11)) setVelY(getPlayerEnt(), 0.015);
                        ...
                        new android.os.Handler().postDelayed(new java.lang.Runnable({
                            run() {
                                try {
                                    ...
                                    var state = getState(mod);
                                    var inst = new android.app.Instrumentation;
                                    if (state && confirmScreenSafe() && !opn && (Entity.getVelX(getPlayerEnt()) != 0 || Entity.getVelZ(getPlayerEnt()) != 0)) {
...
associateModule("Derp,D", [!1, !1], "Player", 0, {
    tick() {
        ...
        if (getState(mod)) {
            Entity.setRot(Player.getEntity(), getYaw() + 30, getPitch());
        }
    }
});
associateModule("NoVoid,NV", [!1, !1], "Player", [{
    ...
    isfalling() {
        ...
        for (var i = getPlayerY(); i > 0; i--) {
            if (Level.getTile(getPlayerX(), i, getPlayerZ()) != 0) arr++;
        };
    ...
    tick() {
        ...
        if (getState(mod)) {
            if (notaccepted.indexOf(Level.getTile(Math.floor(getPlayerX()) + 0.5, getPlayerY() - 1.8, Math.floor(getPlayerZ()) + 0.5)) == -1) {
                getCode(mod).lastpos = [Math.floor(getPlayerX()), Math.floor(getPlayerY()), Math.floor(getPlayerZ())]
...
associateModule("MyCollision,MC", [!1, !1], "Player", [{
    ...
    toggle() {
        ...
        getState(mod) ? Entity.setCollisionSize(getPlayerEnt(), getSettings(mod, "X", 0), getSettings(mod, "Y", 0)) : Entity.setCollisionSize(getPlayerEnt(), 0.6, 1.8)
    }
});
    ...
    getNearestEntity(range) {
        ...
        /*players.forEach((e)=>{
            //if (getCode(getModule("ClickFriends")).isFriend(e) == false) {
                var x = Entity.getX(players[e]) - getPlayerX();
                var y = Entity.getY(players[e]) - getPlayerY();
                var z = Entity.getZ(players[e]) - getPlayerZ();
                var dist = Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2) + Math.pow(z, 2));
    ...
    render(gl) {
        ...
        if (getState(mod)) {
            ...
            players.forEach(function (entry) {
                ...
                if (entry != getPlayerEnt() && Entity.getEntityTypeId(entry) == 63 && getCode(getModule("EntityList")).isValid(entry)) {
                    //Render.drawBox(gl, Entity.getX(entry) - 0.5, Entity.getY(entry) - 0.5, Entity.getZ(entry) - 0.5, 0.9, 1.85, 0.9, getCode(getModule("ClickFriends")).isFriend(entry));
                    Render.drawLine(gl, Entity.getX(entry) - 0.5, Entity.getY(entry) - 0.5, Entity.getZ(entry) - 0.5, getPlayerX(), 0, getPlayerZ(), getCode(getModule("ClickFriends")).isFriend(entry));
                    //getPlayerX(), getPlayerY() - 100, getPlayerZ()
                }
            })
        }
    },
...
associateModule("FakeCam,FC", [!1, !1], "Misc", 0, {
    ...
    rotate() {
        ...
        if (!getCode(getModule("AimBot")).rotating) {
            getCode(mod).lastYaw = Entity.getYaw(getPlayerEnt()) % 360 * Math.PI / 180;
            getCode(mod).lastPitch = Entity.getPitch(getPlayerEnt()) % 180 * Math.PI / 90;
    ...
    /*aim() {
        ...
        if (getCode(mod).fakeEnt != -1) {
            var x = getPlayerX() - Entity.getX(getCode(mod).fakeEnt);
            var y = getPlayerY() - Entity.getY(getCode(mod).fakeEnt);
            var z = getPlayerZ() - Entity.getZ(getCode(mod).fakeEnt);


            var a = 0.5 + getPlayerX();
            var b = getPlayerY();
            var c = 0.5 + getPlayerZ();
            var len = Math.sqrt(x * x + y * y + z * z);
            var y = y / len;
            var pitch = Math.asin(y);
            pitch = pitch * 180.0 / Math.PI;
    ...
    aim() {
        var mod = getModule("FakeCam");
        /*var x = getPlayerX() - getCode(mod).lastpos[0];
        var y = getPlayerY() - 2 - getCode(mod).lastpos[1];
        var z = getPlayerZ() - getCode(mod).lastpos[2];
        ...
        //;
        // Entity.setRot(getCode(mod).fakeEnt, Entity.getYaw(getPlayerEnt()) / 2,Entity.getPitch(getPlayerEnt()));
    ...
    avr() {
        ...
        if (getState(mod) && confirmScreenSafe()) {
            if (getCode(mod).fakeEnt != -1) {
                ...
                getCode(mod).aim();
                /* if(!getCode(getModule("AimBot")).rotating){
                    Entity.setRot(getCode(mod).fakeEnt, Entity.getYaw(getPlayerEnt()), 0);
                    getCode(mod).lastYaw = Entity.getYaw(getPlayerEnt())
                    }else{
    ...
    button() {
        ...
        uithread(() => {
            try {
                ...
                getCode(mod).btn.setText(android.text.Html.fromHtml('<font color="white">X:</font> ' + Math.round(Player.getX()) + " <font color='white'>Y:</font> " + Math.round(Player.getY()) + " <font color='white'>Z:</font> " + Math.round(Player.getZ())));
                getCode(mod).btn.setTextColor(android.graphics.Color.WHITE);
                getCode(mod).btn.setTextSize(gs(17));
                getCode(mod).btn.setGravity(android.view.Gravity.RIGHT);
                getCode(mod).btn.setTypeface(android.graphics.Typeface.create("sans-serif", android.graphics.Typeface.BOLD));
...
if (Halcyon.isDev) {
    ...
    /*
    associateModule("ESP2D,MC", [!1, !1], "Misc", 0, {
        toggle() {
                                    ...
                                    try {
                                        if (getState(mod) && sscreen.indexOf("chat") != -1 && !opn){
                                            ...
                                            players.forEach(function (e) {
                                                ...
                                                if (e != getPlayerEnt() && !getCode(getModule("ClickFriends")).isFriend(e) && e != -1 && ename != undefined) {
                                                    
                                                    com.mojang.minecraftpe.MainActivity.currentMainActivity.get().nativeSetTextboxText("/w "+ ename +" ! - Halcyon: vk: null");
                                                    com.mojang.minecraftpe.MainActivity.currentMainActivity.get().updateTextboxText("/w "+ ename +" ! - Halcyon: vk: null");

                                                  // com.mojang.minecraftpe.MainActivity.currentMainActivity.get().nativeSetTextboxText("! - Halcyon: vk: null");
                                              //   com.mojang.minecraftpe.MainActivity.currentMainActivity.get().updateTextboxText("! - Halcyon: vk: null");
    ...
    //var a = "§7§l[§6Gold§7]§a GoldyYt"
     //console.log(a.replace(/§1/g, "").replace(/§2/g, "").replace(/§3/g, "").replace(/§f/g, "").replace(/§5/g, "").replace(/§6/g, "").replace(/§7/g, "").replace(/§8/g, "").replace(/§9/g, "").replace(/§0/g, "").replace(/§a/g, "").replace(/§b/g, "").replace(/§c/g, "").replace(/§d/g, "").replace(/§e/g, "").replace(/§f/g, "").replace(/§l/g, "").replace(/§k/g, "").replace(/§o/g, "").replace(/§r/g, ""))
        toggle() {
            try {
                var mod = getModule("TestSpammer");
                if (getCode(mod).toggled == false) {
...
var customMessage = (msg) => {
    clientMessage("§f§lH§falcyon §7>§r " + msg)
}
        ...
        init: function () {
            ...
            this.renderer = new android.opengl.GLSurfaceView.Renderer({
                ...
                onSurfaceChanged: function (gl, width, height) {
                    ...
                    gl.glMatrixMode(GL10.GL_PROJECTION);
                    gl.glLoadIdentity();
                    android.opengl.GLU.gluPerspective(gl, Render_tracers.Render.fov, width / height, -0.99, 99.9);
                    gl.glMatrixMode(GL10.GL_MODELVIEW);
                    gl.glLoadIdentity();
                },
                onDrawFrame: function (gl) {
...
var watermark = () => {
    uithread(() => {
        try { //.getPaint().setShader(new android.graphics.LinearGradient(0, 200, 0, 0, [Rainbow((entry.button.getId()) * 200), Rainbow((entry.button.getId() + 2) * 200)], null, android.graphics.Shader.TileMode.MIRROR))
            ...
            pbt.setText(android.text.Html.fromHtml("Remake by <font color='#ffd900'>Hayanderu</font>"));
    ...
    if (getState(mod)) uithread(() => {
        try {
            ...
            nots.push({
                ...
                gui: null,
                index: lastindex,
                pos: {x:0,y:gs(dip2px(15)) + 30 + lastindex * gs(dip2px(30))}
            });
...
var pos = () => {
    uithread(() => {
        try {
            post.setText(android.text.Html.fromHtml('<font color="white">X:</font> ' + Math.round(Player.getX()) + " <font color='white'>Y:</font> " + Math.round(Player.getY()) + " <font color='white'>Z:</font> " + Math.round(Player.getZ())));
...
var booleansetting = (setting) => {
    try {
        ...
        if (setting.values)
            ...
            text.setText(android.text.Html.fromHtml(setting.name + ': <font color="white">false</font>'));
        text.setTextColor(android.graphics.Color.parseColor("#e1e4eb"));
        text.setTextSize(gs(15));
        text.setTypeface(android.graphics.Typeface.create("sans-serif", android.graphics.Typeface.BOLD));
        text.setPadding(0, 0, 0, gs(4));
...
var settings = (mod) => {
    uithread(() => {
        try {
            ...
            text_main.setText(android.text.Html.fromHtml('<font color="#FFFFFF">' + name1 + '</font><font color="#ffffff">' + name2 + '</font>'));
            ...
            let line = new android.widget.LinearLayout(ctx);
            line.setBackgroundColor(android.graphics.Color.parseColor("#FFFFFF"));
            line.setLayoutParams(params);
            lay.addView(line);
            ...
            if (mod.state[1])
                ...
                text_main.setText(android.text.Html.fromHtml('Bind: <font color="white">' + getBindState(mod) + '</font>'));
            text_main.setTextColor(-1);
            ...
            gui.setAnimationStyle(android.R.style.Animation_Toast);
...
var sbar = (mod) => {
    ...
    btn.setOnClickListener(new android.view.View.OnClickListener({
        onClick: function (v) {
            ...
            getState(mod) ? notification("<font color='green'>Enabled:</font> <font color='white'>" + mod.name[0] + "</font>") : notification("<font color='white'>Disabled:</font> <font color='white'>" + mod.name[0] + "</font>")
    ...
    ctx.runOnUiThread({
        run() {
            try {
                ...
                btn.setOnTouchListener(new android.view.View.OnTouchListener({
                    onTouch: function (view, motionEvent) {
                        try {
                            ...
                            switch (motionEvent.getAction()) {
                                case android.view.MotionEvent.ACTION_DOWN:
                                    btn.setBackground(module.state[0] ? style.custom_corner("#696969", [gs(20), gs(20), gs(20), gs(20), gs(20), gs(20), gs(20), gs(20)]) : style.custom_corner("#2d2d2d", [gs(20), gs(20), gs(20), gs(20), gs(20), gs(20), gs(20), gs(20)]));
                                ...
                                case android.view.MotionEvent.ACTION_UP:
                                    ...
                                    if (!moving) {
                                        ...
                                        if (module.code.hasOwnProperty('toggle')) module.code.toggle();
                                        btn.setBackground(module.state[0] ? style.mainon() : style.main());
                                        getState(module) ? notification("<font color='green'>Enabled:</font> <font color='white'>" + module.name[0] + "</font>") : notification("<font color='white'>Disabled:</font> <font color='white'>" + module.name[0] + "</font>")
                                    };
                                    break;
                                case android.view.MotionEvent.ACTION_CANCEL:
                                    break;
                ...
                module.bind.setAnimationStyle(android.R.style.Animation_Toast);
        ...
        save.setOnClickListener(new android.view.View.OnClickListener({
            onClick(v) {
                if (edittext.getText() != "" || choosen != "") {
                    ...
                    notification("<font color='white'>Error!</font> <font color='white'>Please, enter a valid name!</font>")
                };
            }
        }));
        right.addView(save);
    ...
    unban.setOnClickListener(new android.view.View.OnClickListener({
        onClick(v) {
            try {
                ...
                print("Restarting your launcher.");
                rest()
            } catch (e) {
                errorMessage(e)
            }
...
var menu = () => {
    uithread(() => {
        try {
            ...
            text_main = new android.widget.TextView(ctx);
            //Halcyon.isDev ? text_main.setText(android.text.Html.fromHtml('<font color="' + maincolor + '">H</font><font color="#ffffff">alcyon - build 7 Dev</font>')) : text_main.setText(android.text.Html.fromHtml('<font color="#FFFFFF">H</font><font color="#ffffff">alcyon - build 7 Alpha</font>'))
            //text_main.setText(android.text.Html.fromHtml('<font color="' + maincolor + '">H</font><font color="#ffffff">alcyon - build '+Halcyon.build+'</font>'))
            ...
            text_main1.setText(android.text.Html.fromHtml('null'));
            ...
            popup_menu.setAnimationStyle(android.R.style.Animation_Toast);
            ...
            run() {
                ...
                try {
                    ...
                    if (oT.includes("key_key.attack:-99") || !oT.includes("key_key.attack:81")) {
                        ...
                        fos.close();
                        customMessage("Settings replaced. Restarting your MCPE");
...
var attackHook = (a, v) => {
    try {
        ...
        if(a == getPlayerEnt()){
            clientMessage(clearName(Entity.getNameTag(v)))
        }
    } catch (e) {
        errorMessage(e)
    }
...
var serverMessageReceiveHook = (msg) => {
    ...
    if (a.total != 0) {
        notification("<font color='white'>Warning!</font> <font color='white'>Someone called you are hacker!</font>")
    };
    a.total = 0;
...
var clearName = (name) => {
    if(name != -1 && name != null && name != "" && name != undefined)
      return name.replace(/§1/g, "").replace(/§2/g, "").replace(/§3/g, "").replace(/§f/g, "").replace(/§5/g, "").replace(/§6/g, "").replace(/§7/g, "").replace(/§8/g, "").replace(/§9/g, "").replace(/§0/g, "").replace(/§a/g, "").replace(/§b/g, "").replace(/§c/g, "").replace(/§d/g, "").replace(/§e/g, "").replace(/§f/g, "").replace(/§l/g, "").replace(/§k/g, "").replace(/§o/g, "").replace(/§r/g, "")
else
      return "0"
}