...
if (Halcyon.isDev) {
    ...
    associateModule("SaintBypass <font color='#ffd900'>Premium</font>,+", [!1, !1], "Misc", 0, {
        old: 0,
        state: true,
        toggle() {
            var mod = getModule("SaintBypass <font color='#ffd900'>Premium</font>");
            if (getState(mod)) {
                getCode(mod).old = getPlayerX();
                //setPosition(getPlayerEnt(), getPlayerX() + 1000, getPlayerY(), getPlayerZ());
            } else {
                setPosition(getPlayerEnt(), getCode(mod).old, getPlayerY(), getPlayerZ());
            }
        },
        tick() {
            var mod = getModule("SaintBypass <font color='#ffd900'>Premium</font>");
            if (getState(mod)) {
                if (getCode(mod).state) {
                    setPosition(getPlayerEnt(), getPlayerX() + 4096, getPlayerY(), getPlayerZ());
                    getCode(mod).state = !getCode(mod).state;
                } else {
                    setPosition(getPlayerEnt(), getPlayerX() - 4096, getPlayerY(), getPlayerZ());
                    getCode(mod).state = !getCode(mod).state;
                }
